...
from mmt.athlete import athlete_lambda_function
...
def test_create_athlete(athlete_data):
    with patch.object(athlete_lambda_function, "table") as mock_table:
        mock_table.put_item = MagicMock()
        athlete = athlete_lambda_function.create_athlete(**athlete_data)
...
def test_get_athlete(athlete_data):
    with patch.object(athlete_lambda_function, "table") as mock_table:
        mock_table.get_item = MagicMock(return_value={"Item": athlete_lambda_function.Athlete(**athlete_data).to_item()})
        athlete = athlete_lambda_function.get_athlete(athlete_data["athlete_id"])
...
def test_update_athlete(athlete_data):
    with patch.object(athlete_lambda_function, "table") as mock_table:
        mock_table.get_item = MagicMock(return_value={"Item": athlete_lambda_function.Athlete(**athlete_data).to_item()})
        mock_table.put_item = MagicMock()
        athlete = athlete_lambda_function.update_athlete(athlete_data["athlete_id"], nome="Maria", data_nascimento="1992-02-02", sexo="F")
...
def test_delete_athlete(athlete_data):
    with patch.object(athlete_lambda_function, "table") as mock_table:
        mock_table.get_item = MagicMock(return_value={"Item": athlete_lambda_function.Athlete(**athlete_data).to_item()})
        mock_table.put_item = MagicMock()
        athlete = athlete_lambda_function.delete_athlete(athlete_data["athlete_id"])
...
def test_lambda_handler_create(athlete_data):
    with patch.object(athlete_lambda_function, "table") as mock_table:
        ...
        result = athlete_lambda_function.lambda_handler(event, None)
